...
from Functions import run_function_dbd_scheduling_cost_min_ref_2
from Functions import neighborhood_k_eq_2,solve_subproblem,generate_initialization
...
def build_scheduling_Boolean_cost_min():
    ...
    m.S=pe.Var(m.K,m.T,within=pe.NonNegativeReals,initialize=0,bounds=_S_bounds,doc='Inventory of material k at time t')
    # m.S.display()
...
def build_scheduling_Boolean_cost_min_relaxed():
    ...
    #VARIABLES------------------ 
    m.X=pe.Var(m.I,m.J,m.T,within=pe.NonNegativeReals,initialize=0,bounds=(0,1),doc='1 if unit j processes task i starting at time t')   
    # help(pe.Var)
    m.B=pe.Var(m.I,m.J,m.T,within=pe.NonNegativeReals,initialize=0,doc='Batch size of task i processed in unit j starting at time t')
    def _S_bounds(m,K,T):
        return (None,m.gamma[K])
...
if __name__ == "__main__":
    ...
    initialization=[15,8,23,71,1,45,1,9] # TODO: Automatically round
    infinity_val=1e+6
